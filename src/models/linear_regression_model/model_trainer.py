...
class ModelTrainer:
    ...
    def _mlflow_log_artifacts_and_model(self, saved_path: str, model_type: str):
        ...
        # 1) Subir el .pkl versionado
        try:
            mlflow.log_artifact(saved_path)
        except Exception as e:
            print(f"[WARN] No se pudo loguear artifact .pkl: {e}")

        # 2) Guardar temporalmente el modelo en formato MLflow y subirlo como artifacts
        try:
    ...
    def save_model(self, model_type="linear_regression", timestamp=None):
        ...
        """
        import os as _os, datetime as _dt, joblib as _joblib
        if timestamp is None:
            timestamp = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
        versioned_dir = f"models/{model_type}/artifacts"
        _os.makedirs(versioned_dir, exist_ok=True)
        versioned_model_path = _os.path.join(versioned_dir, f"model_{timestamp}.pkl")
        _joblib.dump(self.model, versioned_model_path)
    ...
    def run(self, X_train, X_test, y_train, y_test, model_type="linear_regression", timestamp=None):
        ...
        tracking_uri = env_vars.get("MLFLOW_TRACKING_URI") or os.getenv("MLFLOW_TRACKING_URI")
        if tracking_uri:
            mlflow.set_tracking_uri(tracking_uri)

        parsed = urlparse(mlflow.get_tracking_uri())
        if parsed.scheme.startswith("postgres"):
            print(f"[WARN] Tracking URI apunta a DB ({mlflow.get_tracking_uri()}). Corrigiendo a http://127.0.0.1:5001")
            mlflow.set_tracking_uri("http://127.0.0.1:5001")

        base_exp = env_vars.get("EXPERIMENT_NAME", "steel-energy")
        lin_exp = os.getenv("LIN_EXPERIMENT_NAME", f"{base_exp}-lin")
        mlflow.set_experiment(lin_exp)
        # ------------------------------------------------------------------------------
        ...
        try:
            ...
            # 3) Guardar métricas para DVC + log a MLflow
...
class PipelineModelTrainer(ModelTrainer):
    ...
    def __init__(
        ...
        *,
        ...
        # Import perezoso para evitar ciclos si experiment_pipelines importa trainers
